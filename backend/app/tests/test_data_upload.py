...
def auth_token(client: "TestClient"):
    ...
    response = client.post(
        "/auth/register",
        json={"username": "uploader", "email": "uploader@example.com", "password": "uploadpassword"}
    )
...
def test_upload_dataset(client: "TestClient", auth_token: str):
    ...
    files = {
        "file": ("test_dataset.csv", StringIO(file_content), "text/csv")
    }
...
def test_upload_dataset_invalid_file_type(client: "TestClient", auth_token: str):
    ...
    files = {
        "file": ("invalid_file.html", StringIO(file_content), "text/html")
    }
    response = client.post("/data/upload", data=data, files=files, headers=headers)
...
def test_upload_dataset_unauthorized(client: "TestClient"):
    ...
    files = {
        "file": ("unauthorized_dataset.csv", StringIO(file_content), "text/csv")
    }
